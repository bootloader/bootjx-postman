...
@Component
@ConnectorMapping(contactType = ContactType.PUSH, channel = CHANNEL_TYPE.PUSHAPP)
public class PushAppConnector extends AbstractConnector<OAConfigDetails, OAPlugin> {

	private static Logger LOGGER = LoggerService.getLogger(PushAppConnector.class);

	@Value("${mry.pushapp.url:}")
	...
	public Status onSend(ChannelConfig channelConfig, ChatContactDoc chatContactDoc, OutboxMessage outboxMessage,
		...
		try {
			restService.ajax(pushappUrl).path("/pushapp/api/v1/message/push")
					.header("x-app-id", channelConfig.getPushapp().getAppId())
					.header("x-app-key", channelConfig.getPushapp().getAppKey()).postJson(outBoundMsg).asMap();
